  ...
  return (
    <footer className="bg-DarkBlue text-gray-200 relative overflow-hidden">
        ...
        <div className="relative z-10">
            ...
            <div className="pt-20 pb-28" style={{ backgroundColor: 'rgba(0,0,0,0.1)' }}>
            ...
            <div className="container mx-auto px-4 pb-8 mt-1">
                <div className="flex flex-col lg:flex-row max-w-7xl justify-between items-center lg:items-start text-center lg:text-left mx-auto">
                    ...
                    <p className="text-lg mb-2 relative top-16 left-[72px]"> In partnership with:</p>
                        <img src="white_logo.png" alt="WSB Cargo & Logistics" className="h-auto w-64 mt-4 mb-0 rounded-xl " />
                        <div className="flex space-x-6 mt-0  lg:mb-0 lg:mt-0 mb-6">
                            ...
                            <a href={contactInfo.social.twitter} target="_blank" rel="noopener noreferrer" className="bg-white p-2 rounded-full transition-transform hover:scale-110"><FontAwesomeIcon icon={faXTwitter} className="text-DarkBlue text-3xl" /></a>
                        </div>
                    </div>
                            ...
                            <div className="flex items-start space-x-4">
                                ...
                                <div>
                                    {contactInfo.address.map((line, index) => (
                                        <React.Fragment key={index}>{line}{index < contactInfo.address.length - 1 && <br />}</React.Fragment>
                                    ))}
                                </div>
                            </div>
                            <div className="flex items-start space-x-4">
                                ...
                                <div>
                                    <a href={`tel:${contactInfo.phones[0].replace(/\s/g, '')}`} className="hover:underline block">{contactInfo.phones[0]}</a>
                                    <a href={`tel:${contactInfo.phones[1].replace(/\s/g, '')}`} className="hover:underline block">{contactInfo.phones[1]}</a>
                                </div>
                            </div>
                            <div className="flex items-start space-x-4">
            ...
            <div className="container mx-auto border-t border-gray-600/50 mt-4 py-6 text-center text-sm text-gray-400">
                <p>© {new Date().getFullYear()} WSB Cargo & Logistics. All Rights Reserved.</p>
            </div>
        </div>
    </footer>
  );
